...
class Tag():
    ...
    def __fromBin__(self, data):
        ...
        # Text fields
        self.tagVersion = int(toStr(fields[0])) #4
        self.signature1 = toStr(fields[1]) #20
        self.signature2 = toStr(fields[2]) #14
        self.chipID     = int(toStr(fields[3])) #6
        self.boardID    = toStr(fields[4]) #16
        self.bigEndian  = bool(int(toStr(fields[5]))) #2
        self.imageLen   = int(toStr(fields[6])) # 10
        self.cfeAddr    = int(toStr(fields[7])) # 12
        self.cfeLen     = int(toStr(fields[8])) # 10
        self.rootfsAddr = int(toStr(fields[9])) # 12
        self.rootfsLen  = int(toStr(fields[10])) # 10
        self.kernelAddr = int(toStr(fields[11])) # 12
        self.kernelLen  = int(toStr(fields[12])) # 10
        self.imageSeq   = toStr(fields[13]) # 4
        self.imageVer   = toStr(fields[14]) # 32
        self.reserved   = fields[15] # 42
        # Binary fields
        self.imageToken = fields[16] # 20
        self.tagToken   = fields[17] # 20        

        # Extract JamCRC from binary fields
        (self.imageCRC,
        self.rootfsCRC,
        self.kernelCRC,
        self.dtbCRC, self.post)           = self.endianUnpack("5I", self.imageToken)

        (self.tagCRC,
        self.tagCRC1, self.tagCRC2, self.tagCRC3, self.tagCRC4)     = self.endianUnpack("5I", self.tagToken)
    ...
    def __str__(self):
        ...
        return "Tag Version:        " + str(self.tagVersion)       + "\n" \
               ...
               "DTB jamCRC:         " + str(hex(self.dtbCRC))      + "\n" \
